...
serve(async (req) => {
  ...
  try {
    ...
    const identifier = token.slice(0, 20);
    const rlSupabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
    const windowStart = new Date(Date.now() - 60_000).toISOString();
    const { data: rlData } = await rlSupabase.from("rate_limits").select("id, request_count").eq("identifier", identifier).eq("function_name", "create-checkout").gte("window_start", windowStart).maybeSingle();
    if (rlData && rlData.request_count >= 5) {
      return new Response(JSON.stringify({ error: "Rate limit exceeded" }), { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (rlData) {
      await rlSupabase.from("rate_limits").update({ request_count: rlData.request_count + 1 }).eq("id", rlData.id);
    } else {
      await rlSupabase.from("rate_limits").insert({ identifier, function_name: "create-checkout", request_count: 1 });
    }
